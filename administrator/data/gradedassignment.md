# Graded Assignment

<iframe width="560" height="315" src="https://www.youtube.com/embed/8UwlrhjRVuM" title="YouTube video player" frameborder="0" allowfullscreen></iframe>

## Description

When Agentic OS is embedded in Canvas as an LTI tool, it can grade learner responses inside an Assignment and automatically pass the score back to the Canvas gradebook. Learners type their answers directly in the agent chat, receive an instant score (and, soon, AI-generated feedback), and watch their grade update in real time—no manual grading steps required.

---

## Target Audience

**Administrator**

---

## Features

#### Instant AI Scoring

The agent evaluates each submitted answer and returns a numeric grade (e.g., 10 / 10).

#### Automatic Grade Passback

Scores sync to the Canvas gradebook immediately after each response.

#### Iterative Resubmission

Every new answer replaces the previous score (e.g., a perfect 10 drops to 0 if the next attempt is wrong).

#### Future Enhancements (preview)

- AI-generated feedback accompanying each grade  
- Support for file uploads (PDF, Word, Excel, etc.) that the agent can grade

---

## How to Use (step by step)

#### Open the Assignment

- In Canvas, navigate to **Assignments**  
- Click the task labeled (for example) **Agent Grading**

#### Interact with the Agent

- The agent chat loads on the assignment page  
- Type your answer (e.g., `5 + 5 = 10`) into the chat and press **Send**

#### Receive Your Score

- The agent instantly replies with a grade (e.g., **10 / 10**)  
- *(Future)* Feedback text will appear alongside the score

#### Verify Grade Passback

- Open **Grades** in Canvas  
- Confirm the score has updated (e.g., **10 / 10**)

#### Revise & Resubmit (Optional)

- Return to the assignment page  
- Enter a new answer (e.g., `5 + 5 = 9`)  
- The agent re-grades and returns **0 / 10**  
- Check **Grades** again to see the new score reflected

---

## Pedagogical Use Cases

#### Math & Short-Answer Quizzes

Verify calculations or factual responses instantly, freeing administrators from manual grading.

#### Formative Practice

Allow users **unlimited retries**; the most recent attempt determines the recorded grade.

#### Low-Stakes Knowledge Checks

Integrate quick “exit ticket” questions in modules, providing **immediate performance feedback**.

#### Future File-Based Assessments

Upload **lab worksheets, essays, or spreadsheets** for AI scoring once file-grading launches.

---

With Agentic OS’s Canvas integration, learners receive **live grades** (and soon feedback) while administrators enjoy **automatic score passback**—streamlining assessment and reinforcing learning in real time.
