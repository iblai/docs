# Link Share

<iframe width="560" height="315" src="https://www.youtube.com/embed/7RO3QsjqOL4" title="YouTube video player" frameborder="0" allowfullscreen></iframe>

## Description

The Link Share feature generates a public URL that lets anyone chat with your Agentic OS—no login required. Toggle the link on to invite open access, refresh it to rotate the URL, or switch it off to revoke access instantly. Perfect for demos, outreach, or quick external support.

---

## Target Audience

**Administrator**

---

## Features

#### Anonymous Access Toggle

Enable or disable a public, no‑login chat link at any time.

#### One‑Click URL Refresh

Rotate the link with a **refresh icon** to invalidate old URLs.

#### Copy & Distribute

Grab the link and share it via **email, LMS announcements, social media**, or **QR codes**.

#### Instant Lockdown

Turning the toggle **off immediately blocks anonymous sessions**—even if users still have the old link.

#### Safe Testing

Verify the link in an **incognito window** to ensure it works exactly as intended.

---

## How to Use (step by step)

#### Enable Anonymous Access

- Open the agent’s **Embed** tab  
- Locate the **Sharable Link** section  
- Toggle the switch **On** to activate anonymous chatting  
- *(Optional)* Click the **refresh icon** to generate a new URL  
- Click **Copy** and distribute the link wherever you like

#### Test the Link

- Paste the URL into an **incognito/private‑browsing window**  
- Confirm you are **not logged in**  
- The agent chat should load and respond normally for **anonymous users**

#### Disable Anonymous Access

- Return to the **Embed** tab  
- Toggle the switch **Off**  
- Anonymous users will now see:  
  > **Session failed to start**  
  *(Chat box becomes inactive.)*

---

## Pedagogical Use Cases

#### Open‑House Demos

Share a public link during **webinars or campus tours** so prospective users can try the agent without creating accounts.

#### Community Outreach & Marketing

Post the URL on **social channels** to showcase your **AI tutoring capabilities**.

#### Conference or Workshop Support

Provide attendees with instant access to a **specialized agent** for live Q&A during events.

#### Quick External Feedback

Invite **industry partners or subject‑matter experts** to interact with an agent and suggest improvements—no onboarding required.

#### Emergency Helpdesk

Temporarily open access during **downtime or high‑traffic periods**, then disable the link once normal support channels resume.

---

With **Link Share**, you control whether outsiders can interact with your agent—**turn the link On for open access**, or **flip it Off to lock things down instantly**.
