# Profile Management

<iframe width="560" height="315" src="https://www.youtube.com/embed/EwxEElHjH-c" title="YouTube video player" frameborder="0" allowfullscreen></iframe>

## Description

The **User Profile** lets you manage the personal details that the agent uses to personalize your experience—**name, contact info, preferred language, social links, education, work history, and résumé**. You can also **reset your password** from here.  

Profile updates help Agentic OS **tailor guidance** (e.g., career advice) and help **instructors/admins** understand learner context.

---

## Target Audience

**User**

---

## Features

#### Basic Information
Edit your **full name, email, title**, and an **About blurb** summarizing your background or interests.

#### Preferred Language
Choose your **interface language** (e.g., Spanish, French).  
The agent adapts the UI and experience accordingly.

#### Social
Add optional links (Facebook, LinkedIn, X) to make it easier to connect outside the platform.

#### Education
Record details such as:
- Degree type  
- Field of study  
- Institution  
- Start/End dates (or mark *In progress*)  
- GPA (optional)

This data is shared with **Agentic OS and superadministrators** to contextualize support.

#### Experience
Capture professional roles (past/current):
- Employer name  
- Role/title  
- Responsibilities/notes  
- Role dates (*mark Currently work here* when applicable)

#### Resume
Upload a résumé/CV (file) for agents and admins to reference—useful for **career guidance** and **tailored recommendations**.

#### Security
Reset your password while logged in; a verification email is sent to complete the change.

---

## How to Use (step by step)

#### Open Your Profile
- Click your **profile icon** (top-right) → select **Profile**  
- You’ll see tabs for **Basic**, **Social**, **Education**, **Experience**, **Resume**, and **Security**

#### Update Basic Information
- Enter **Full name**, **Email**, **Title**, and **About**  
- Set **Preferred language** to localize the platform

#### Add Social Links (Optional)
- In **Social**, paste URLs for Facebook, LinkedIn, or X (or leave blank)

#### Add Education
- In **Education**, click **Add** and fill in:
  - Degree type  
  - Field of study  
  - Institution  
  - Start/End dates  
  - *(Optional)* Mark **In progress** or add **GPA**

#### Add Experience
- In **Experience**, click **Add** and enter:
  - Employer  
  - Role/title  
  - Responsibilities/notes  
  - Dates of employment  
  - Toggle **Currently work here** when applicable

#### Upload Your Resume
- Go to **Resume** → click **Upload** to add your résumé (PDF/DOC)  
- The file is shared with admins and Agentic OS to personalize support

#### Reset Your Password (If Needed)
- Open **Security** → click **Reset password**  
- Check your email for the verification link and complete the change

#### Save & Verify
- Ensure each tab’s changes are **Saved**  
- Reopen your agent chat to benefit from updated personalization

---

## Pedagogical Use Cases

#### Personalized Career Guidance
Education/experience + résumé help the agent suggest roles, skills to build, and tailored application tips.

#### Language Accessibility
The **Preferred Language** ensures the UI and responses match your preferred communication language.

#### Advising & Support Context
Instructors/admins can view education progress and interests to better target resources.

#### Showcasing Achievements
A complete profile (GPA, projects, roles) improves the relevance of **scholarships, internships, and portfolio feedback**.

---

Keep your **User Profile** current to get the most **accurate, relevant, and personalized help** from Agentic OS—across courses, career prep, and daily learning.
