...
> Mirrored from [`iblai/os`](https://github.com/iblai/os) · [`docs/development.md`](https://github.com/iblai/os/blob/main/docs/development.md). This page is generated — edit it in the repository, not here.
...
The exact `@iblai/iblai-js` version this app is tested against lives in [`.iblai-js-version`](https://github.com/iblai/os/blob/main/.iblai-js-version). The pre-push hook checks `package.json` against it; bump both when you intentionally move SDK versions, otherwise leave it alone.
...
| Monitoring  | Sentry                                                                                                                           |
| SDK         | [@iblai/iblai-js](https://www.npmjs.com/package/@iblai/iblai-js) (version pinned in [`.iblai-js-version`](https://github.com/iblai/os/blob/main/.iblai-js-version)) |

---

## AGENTS.md

[`AGENTS.md`](https://github.com/iblai/os/blob/main/AGENTS.md) at the repo root documents the formatting, lint, and push-protocol rules that automated agents (Claude Code, OpenCode, etc.) follow when editing this codebase. Read it before you start contributing — it's also what the husky hooks enforce. `CLAUDE.md` is a symlink to it for tools that still look for that filename.

The skills it references live under `.claude/skills/`:

- [`.claude/skills/prettier-format.md`](https://github.com/iblai/os/blob/main/.claude/skills/prettier-format.md) — formatter rules and the pre-commit hook
- [`.claude/skills/safe-push.md`](https://github.com/iblai/os/blob/main/.claude/skills/safe-push.md) — push protocol; never bypass the pre-push hook
- [`.claude/skills/e2e-coverage.md`](https://github.com/iblai/os/blob/main/.claude/skills/e2e-coverage.md) — when `e2e/coverage.json` needs an update
