...
> **Next step:** Once a `ConnectedService` exists, wire it into an MCP server with the [MCP Server Connections](/developer/platform/mcp/mcp-connections) guide, or let a learner connect inline via [In-Chat MCP Events](/developer/platform/mcp/in-chat-mcp-events).
