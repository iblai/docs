...
> Mirrored from [`iblai/claw-setup`](https://github.com/iblai/claw-setup) · [`docs/platform-integration.md`](https://github.com/iblai/claw-setup/blob/main/docs/platform-integration.md). This page is generated — edit it in the repository, not here.
...
| `https://api.iblai.app/dm` | `/dm/api/ai-mentor/orgs/<org>/…` | Requires `/dm`. This is the `IBLAI_HOST` default in [`install.sh`](https://github.com/iblai/claw-setup/blob/main/install.sh) and [`scripts/seed_claw_mentor.py`](https://github.com/iblai/claw-setup/blob/main/scripts/seed_claw_mentor.py). |
...
| `gateway_token` | string | Write-only. The token from [server setup step 1.3](/developer/guides/openclaw-server-setup#13-generate-a-gateway-token) |
...
See [server setup step 5.2](/developer/guides/openclaw-server-setup#52-generate-and-store-device-keypair) for how to generate the keypair.
...
This only needs to be done once per platform connection. See [Device Re-Pairing](/developer/guides/openclaw-server-setup#device-re-pairing-after-gateway-restarts--updates) if pairing is lost after updates.
...
> [!NOTE]
> Skills push without any extra setup on the worker. Installing the optional `iblai-openclaw-extensions` plugin adds per-agent isolation and clean removal of unassigned skills. Without it, skills install worker-wide and unassigned skills are disabled rather than deleted. See [OpenClaw plugin setup](/developer/guides/openclaw-server-setup#optional-iblai-extensions-plugin-per-agent-skills) or [NemoClaw plugin setup](/developer/guides/nemoclaw-server-setup#optional-iblai-extensions-plugin-per-agent-skills).
