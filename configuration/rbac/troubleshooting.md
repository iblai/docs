...
1. Go to the mentor's **Access** tab.
...
5. The second result shows which **mentor** the policy resource number maps to.
6. Confirm the mentor number in the policy matches the correct mentor.
...
- The **mentor number** can be verified via the Network tab's `check` endpoint.
- Ensure both the platform and mentor resource numbers in the policy match the intended targets.
